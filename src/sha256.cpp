...
std::string sha256(const std::string& input) {
  ...
  for (std::size_t off=0; off<data.size(); off+=64) {
    std::array<std::uint32_t,64> w{}; for (int i=0;i<16;++i) w[i]=(static_cast<std::uint32_t>(data[off+4*i])<<24)|(static_cast<std::uint32_t>(data[off+4*i+1])<<16)|(static_cast<std::uint32_t>(data[off+4*i+2])<<8)|data[off+4*i+3];
    ...
    for (int i=0;i<64;++i) { auto S1=rotr(e,6)^rotr(e,11)^rotr(e,25); auto ch=(e&f)^((~e)&g); auto t1=x+S1+ch+K[i]+w[i]; auto S0=rotr(a,2)^rotr(a,13)^rotr(a,22); auto maj=(a&b)^(a&c)^(b&c); auto t2=S0+maj; x=g;g=f;f=e;e=d+t1;d=c;c=b;b=a;a=t1+t2; }
